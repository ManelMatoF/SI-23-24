...
public class EstrategiaBusquedaA implements EstrategiaBusquedaInformada {
    ...
    public Nodo[] soluciona(ProblemaBusqueda p, Heuristica h) throws Exception {
        ...
        Queue<Nodo> Frontera = new PriorityQueue<>(); //Anchura
        Estado estadoActual = p.getEstadoInicial();
        Nodo nodoActual = new Nodo(estadoActual, null, null, h);
        ...
        while (true) {
            ...
            if (p.esMeta(estadoActual)) break;
                ...
                for (Accion acc : accionesDisponibles) {
                    ...
                    Nodo nodo = new Nodo(sc, nodoActual, acc, h);
                    nodosCreados++;
                    System.out.println((i++) + " - RESULT(" + estadoActual + "," + acc + ")=" + sc);
                    if (!estaExplorado(explorados, sc)) {
                        if (!estaExplorado(Frontera, sc)) {
                            Frontera.add(nodo);
